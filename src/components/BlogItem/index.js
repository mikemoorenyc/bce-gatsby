...
const BlogCopy = ({title,link,excerpt}) => {
   ...
   <div className={blogMeta}> {(excerpt) ? <div className={`${blogExcerpt} ${tagLine}`}>{truncateString(HtmlStrip(excerpt),140)}</div> : null} <Link className={`${blogReadMore} ${fontSans}`} to={link}>Continue reading</Link></div>
    </Fragment>
}
